...
def organizar_csv_completo():
    ...
    with open('dados-divida-ativa-completo.csv', 'w', newline='', encoding='utf-8') as file:
    ...
    import shutil
    shutil.copy('dados-divida-ativa-completo.csv', 'public/dados-divida-ativa-completo.csv')
    
    # Criar estatísticas
    total_valor = sum(d['valor_parcelas'] for d in dados_unicos)
    anos_unicos = sorted(set(d['ano'] for d in dados_unicos if d['ano'] > 0))
    
    print(f"=== RELATÓRIO DE PROCESSAMENTO COMPLETO ===")
...
if __name__ == "__main__":
    dados = organizar_csv_completo()
    print(f"\nArquivo 'dados-divida-ativa-completo.csv' criado com sucesso!")
    print(f"Arquivo copiado para 'public/dados-divida-ativa-completo.csv'")
