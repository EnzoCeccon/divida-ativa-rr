...
def processar_linha_csv_virgulas(linha: str) -> Dict[str, Any]:
    """Processa uma linha do CSV com formato de vírgulas"""
    ...
    try:
        ...
        print(f"Erro ao processar linha com vírgulas: {linha[:100]}... - {e}")
        return None

def processar_linha_csv_tabulacao(linha: str) -> Dict[str, Any]:
    """Processa uma linha do CSV com formato de tabulação"""
    # Divide por tabulações
    campos = linha.split('\t')
    ...
    try:
        ...
        print(f"Erro ao processar linha com tabulação: {linha[:100]}... - {e}")
        return None

def detectar_formato_linha(linha: str) -> str:
    """Detecta se a linha usa vírgulas ou tabulações como separador"""
    if '\t' in linha:
        return 'tabulacao'
    elif ',' in linha:
        return 'virgulas'
...
def organizar_csv_final():
    """Função principal para organizar o arquivo CSV completo"""
    ...
    for i, linha in enumerate(linhas):
        ...
        # Detectar formato da linha
        formato = detectar_formato_linha(linha)
        
        dados = None
        if formato == 'virgulas':
            dados = processar_linha_csv_virgulas(linha)
            if dados:
                linhas_virgulas += 1
        elif formato == 'tabulacao':
            dados = processar_linha_csv_tabulacao(linha)
            if dados:
                linhas_tabulacao += 1
        else:
            print(f"Formato desconhecido na linha {i+1}: {linha[:100]}...")
    ...
    with open('dados-divida-ativa-final.csv', 'w', newline='', encoding='utf-8') as file:
    ...
    import shutil
    shutil.copy('dados-divida-ativa-final.csv', 'public/dados-divida-ativa-final.csv')
    
    # Criar estatísticas
    total_valor = sum(d['valor_parcelas'] for d in dados_unicos)
    anos_unicos = sorted(set(d['ano'] for d in dados_unicos if d['ano'] > 0))
    
    print(f"=== RELATÓRIO DE PROCESSAMENTO FINAL ===")
    print(f"Linhas processadas: {linhas_processadas}")
    print(f"Linhas com vírgulas: {linhas_virgulas}")
    print(f"Linhas com tabulação: {linhas_tabulacao}")
...
if __name__ == "__main__":
    dados = organizar_csv_final()
    print(f"\nArquivo 'dados-divida-ativa-final.csv' criado com sucesso!")
    print(f"Arquivo copiado para 'public/dados-divida-ativa-final.csv'")
